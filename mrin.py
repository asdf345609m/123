...
async def run_attack(chat_id, ip, port, time):
    try:
        ...
        # Notify the user about any errors during execution
        bot.send_message(
            chat_id,
            f"*{str(e)}*",
            parse_mode='Markdown'
...
def handle_attack(message):
    ...
    if user_id in users:
        ...
        if datetime.datetime.now() > expiration_date:
            response = "❗️*𝗬𝗼𝘂𝗿 𝗮𝗰𝗰𝗲𝘀𝘀 𝗵𝗮𝘀 𝗲𝘅𝗽𝗶𝗿𝗲𝗱. 𝗖𝗼𝗻𝘁𝗮𝗰𝘁 𝘁𝗵𝗲 𝗮𝗱𝗺𝗶𝗻 𝘁𝗼 𝗿𝗲𝗻𝗲𝘄*❗️"
            bot.reply_to(message, response, parse_mode='Markdown')
            return
